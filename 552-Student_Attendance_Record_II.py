...
class Solution:
    def checkRecord(self, n):
        ...
        for i in range(3,n + 1):
            # consider ending with distinct eligible suffixes: end with "P": dp[i-1]; end with "PL": dp[i-2]; end with "PLL": dp[i-3]; end with "LLL": is not allowed
            dp[i] = (dp[i - 1] + dp[i - 2] + dp[i - 3] ) % mod
        res = dp[n]
        # with one 'A'
        for i in range(1, n + 1): # insert 'A' at i
