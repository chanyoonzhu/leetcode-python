class SnapshotArray:
    ...
    def set(self, index: int, val: int) -> None:
        self.history[index].append([self.snapshot_id + 1, val])
        return
    ...
    def get(self, index: int, snap_id: int) -> int:
        history = self.history[index]  
        curr_snap_pos = bisect.bisect(history, [snap_id + 1]) - 1 # have to make snap_id + 1 a list to be searchable with bisect
        return history[curr_snap_pos][1]
