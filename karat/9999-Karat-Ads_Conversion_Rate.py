...
class Solution:
    def analyze(self, completed_purchase_user_ids: list, ad_clicks: list, all_user_ips: list):
        users_with_purchase = set(completed_purchase_user_ids)
        ips_with_purchase = set()
        for user_ip in all_user_ips:
            id, ip = user_ip.split(",")
            if id in users_with_purchase:
                ips_with_purchase.add(ip)
        
        clicked = Counter()
        bought = Counter()
        for log in ad_clicks:
            ip, _, text = log.split(",")
            clicked[text] += 1
            if ip in ips_with_purchase:
                bought[text] += 1
        
        return [f"{bought[text]} of {clicked[text]} {text}" for text in clicked.keys()]
